...
get_diagnostics <- function(input_path = '2_process/out/model_summary_results.csv', output_path = '2_process/out/modeldiagnostic_text.txt') {
  ...
  whisker.render(template_1 %>% str_remove_all('\n') %>% str_replace_all('  ', ' '), render_data ) %>% cat(file = file.path(output_path))
